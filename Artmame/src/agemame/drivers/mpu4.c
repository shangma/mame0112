...
static UINT8 chr_data[136];
static UINT8 chr_line8[16];
...
static void ic24_setup(void)
{
	if (IC23GA)
	{
		timer_adjust(ic24_timer, TIME_IN_NSEC(9900000), 0, 0);
	}
}
...
static READ8_HANDLER( pia_ic4_portb_r )
{
	...
	if ( optic_pattern & 0x01 ) ic4_input_b &= ~0x40;
	else                        ic4_input_b |=  0x40; // reel A tab

	if ( optic_pattern & 0x02 ) ic4_input_b &= ~0x20;
	else                        ic4_input_b |=  0x20; // reel B tab

	if ( optic_pattern & 0x04 ) ic4_input_b &= ~0x10;
	else                        ic4_input_b |=  0x10; // reel C tab

	if ( optic_pattern & 0x08 ) ic4_input_b &= ~0x08;
	else                        ic4_input_b |=  0x08; // reel D tab
...
static WRITE8_HANDLER( pia_ic7_porta_w )
{
	...
	if (optic_pattern)
	{
		pia_set_input_cb2(1,0);
		//logerror("OPTIC");
	}
	else
	{
		pia_set_input_cb2(1,2);	// signal is connected to IC4 CB2
	}
}
...
{
	logerror("%04x IC8 PIA Port B Set to %2x (OUTPUT PORT, TRIACS)\n", activecpu_get_previouspc(),data);
}
...
static READ8_HANDLER( pia_gb_porta_r )
{
	LOG(("%04x GAMEBOARD: PIA Read of Port A\n",activecpu_get_previouspc()));
	return 0;
}
...
static READ8_HANDLER( pia_gb_ca1_r )
{
	LOG(("%04x GAMEBOARD: PIA Read of Port CA1\n",activecpu_get_previouspc()));

	return 0;
}

static READ8_HANDLER( pia_gb_ca2_r )
{
	LOG(("%04x GAMEBOARD: PIA Read of Port CA2\n",activecpu_get_previouspc()));

	return 0;
}

static READ8_HANDLER( pia_gb_cb1_r )
{
	LOG(("%04x GAMEBOARD: PIA Read of Port CB1\n",activecpu_get_previouspc()));

	return 0;
}

static READ8_HANDLER( pia_gb_cb2_r )
{
	LOG(("%04x GAMEBOARD: PIA Read of Port CB2\n",activecpu_get_previouspc()));
...
static WRITE8_HANDLER( pia_onb_w )
{
	LOG(("%04x GAMEBOARD: pia write (offset = %d),data = %02X\n", activecpu_get_previouspc(), offset, data));
}

static READ8_HANDLER( pia_onb_r )
{
	LOG(("%04x GAMEBOARD: pia read  (offset = %d)\n", activecpu_get_previouspc(),offset ));
	return 0;
}
...
static const pia6821_interface pia_gameboard_intf =
{
	...
	/*outputs: A/B,CA/B2       */ 0, 0, pia_gb_ca2_w, 0,
	/*irqs   : A/B             */ cpu0_irq,cpu0_irq
};
...
// setup the standard oki MSC1937 display ///////////////////////////////
	...
	vfd_init(0, VFDTYPE_MSC1937,1);	// does oldtimer use a OKI MSC1937 alpha display controller?

	return 0;
}

static MACHINE_START( mpu4old )
{
	pia_config(0, PIA_STANDARD_ORDERING, &pia_ic3_intf);
	pia_config(1, PIA_STANDARD_ORDERING, &pia_ic4_intf);
	pia_config(2, PIA_STANDARD_ORDERING, &pia_ic5_intf);
...
which starts at ff28 and terminates at ff2f for club celebration ff2f then represents the CHR address.
...
static ADDRESS_MAP_START( memmap_d, ADDRESS_SPACE_PROGRAM, 8 )
...
//	AM_RANGE(0x08C0, 0x08C7) AM_READ( ptm6840_2_r)  // 6840PTM for sound board
//	AM_RANGE(0x08C0, 0x08C7) AM_WRITE(ptm6840_2_w)

//	AM_RANGE(0x08E0, 0x08E7) AM_READ( 68681_duart_r) dataport
...
static MACHINE_DRIVER_START( mpu4d )

	MDRV_IMPORT_FROM( mpu4 )
	MDRV_CPU_MODIFY("main")
	MDRV_CPU_PROGRAM_MAP(memmap_d,0)		  			// setup read and write memorymap
	MDRV_MACHINE_START(mpu4d)							// main mpu4 board initialisation
MACHINE_DRIVER_END

static MACHINE_DRIVER_START( mpu4old )
	MDRV_IMPORT_FROM( mpu4 )
	MDRV_MACHINE_START(mpu4old)							// main mpu4 board initialisation
	MDRV_MACHINE_RESET(mpu4_vid)
MACHINE_DRIVER_END

DRIVER_INIT (ccelbrtn)
{
	int x;
	int chr_line8[16]={0x00,0x00,0x01,0x50,0x04,0x00,0x09,0x50,0x10,0x10,0x19,0x40,0x24,0x04,0x31,0x00};
	character_generation_8(0xff30);
	for ( x = 0; x < 16; x++ )
	{
		chr_data[128+x] = chr_line8[(x)];
	}
}
...
//	ROM_LOAD("vid.p1",  0xc000, 0x4000,  CRC(1c8159c4) SHA1(df5363f3fe65b17bb2998399a55b20fb3c5ad144))// The 16k version of the BIOS works just as well.

ROM_START( bctvidbs )
	ROM_REGION( 0x10000, REGION_CPU1, 0 )	/* 16k for BIOS */
	VID_BIOS
ROM_END
...
ROM_START( oldtimer )
	...
	ROM_LOAD( "dot11.bin",  0x0000, 0x10000,  CRC(da095666) )
ROM_END

ROM_START( ccelbrtn )
	ROM_REGION( 0x10000, REGION_CPU1, 0 )	/* 64k for code */
	ROM_LOAD( "cels.p1",  0x0000, 0x10000,  CRC(19d2162f) )
ROM_END
...
GAME( 198?, oldtimer,0,       mpu4d,    mpu4,     0,        0,   "Barcrest", 		"Old Timer",														GAME_NOT_WORKING|GAME_NO_SOUND )
GAME( 198?, ccelbrtn,0,       mpu4,     mpu4,    ccelbrtn,	0,   "Barcrest", 		"Club Celebration",													GAME_NOT_WORKING|GAME_IMPERFECT_SOUND )
