...
static READ8_HANDLER( sc3_expansion_r )
{
	...
	switch ( offset )
	{
		case 0:	result = 0;
		break;
		case 1:	result = input_port_0_r(0);  // scorpion3 coin input
	}
...
static ADDRESS_MAP_START( memmap_sc2_dm01, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0x0000, 0x1fff) AM_READWRITE(ram_r, ram_w) AM_BASE(&nvram) AM_SIZE(&nvram_size)// 8k RAM
	AM_RANGE(0x2000, 0x2000) AM_READ(vfd_status_dm01_r)		// vfd status register
	AM_RANGE(0x2000, 0x20FF) AM_WRITE(reel12_w)
	AM_RANGE(0x2100, 0x21FF) AM_WRITE(reel34_w)
	AM_RANGE(0x2200, 0x22FF) AM_WRITE(reel56_w)
	...
	AM_RANGE(0x2600, 0x2600) AM_READWRITE(uart2stat_r, uart2ctrl_w)	// mc6850 compatible uart
	AM_RANGE(0x2700, 0x2700) AM_READWRITE(uart2data_r, uart2data_w)
	AM_RANGE(0x2800, 0x2800) AM_WRITE(vfd1_data_dm01_w)		// vfd1 data
	AM_RANGE(0x2900, 0x2900) AM_WRITE(vfd_reset_w)			// vfd1+vfd2 reset line
...
// input ports for dr.who /////////////////////////////////////////////////

	INPUT_PORTS_START( drwho )
	PORT_START_TAG("COINS")
	PORT_BIT(0x01, IP_ACTIVE_HIGH, IPT_COIN1 ) PORT_IMPULSE(3)
	PORT_BIT(0x02, IP_ACTIVE_HIGH, IPT_COIN2 ) PORT_IMPULSE(3)
	PORT_BIT(0x04, IP_ACTIVE_HIGH, IPT_COIN3 ) PORT_IMPULSE(3)
...
// input ports for club public enemy no.1  ////////////////////////////////

	INPUT_PORTS_START( cpeno1 )
	PORT_START_TAG("COINS")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_COIN1 ) PORT_IMPULSE(3)
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_COIN2 ) PORT_IMPULSE(3)
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_COIN3 ) PORT_IMPULSE(3)
...
// input ports for luvjub /////////////////////////////////////////////////

	INPUT_PORTS_START( luvjub )
	PORT_START_TAG("COINS")
	PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_COIN1 ) PORT_IMPULSE(3)
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_COIN2 ) PORT_IMPULSE(3)
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_COIN3 ) PORT_IMPULSE(3)
...
// input ports for scorpion3 board ////////////////////////////////////////

INPUT_PORTS_START( scorpion3 )
	PORT_START_TAG("COINS")
    PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_COIN1 ) PORT_IMPULSE(3) PORT_NAME("Fl 5.00")
	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_COIN2 ) PORT_IMPULSE(3) PORT_NAME("Fl 2.50")
	PORT_BIT( 0x04, IP_ACTIVE_HIGH, IPT_COIN3 ) PORT_IMPULSE(3) PORT_NAME("Fl 1.00")
...
static MACHINE_DRIVER_START( scorpion2_dm01 )
	MDRV_MACHINE_RESET(init)							// main scorpion2 board initialisation
	MDRV_INTERLEAVE(8)									// needed for serial communication !!
	MDRV_CPU_ADD_TAG("main", M6809, 2000000 )			// 6809 CPU at 2 Mhz
	MDRV_CPU_PROGRAM_MAP(memmap_sc2_dm01,0)				// setup read and write memorymap
	...
	MDRV_CPU_ADD_TAG("matrix", M6809, 2000000 )			// matrix board 6809 CPU at 2 Mhz ?? I don't know the exact freq. 
	MDRV_CPU_PROGRAM_MAP(bfm_dm01_memmap,0)				// setup matrix board memorymap
	MDRV_CPU_PERIODIC_INT(bfm_dm01_vbl, TIME_IN_HZ(1500) )// generate 1500 NMI's per second ?? what is the exact freq??
MACHINE_DRIVER_END

static void sc2_common_init(int reels)
{
	UINT8 *rom, i;
...
DRIVER_INIT (drwho)
{
	...
	Scorpion2_SetSwitchState(4,0, 0);	  // £1  Low Level Switch
	...
	Scorpion2_SetSwitchState(7,0, 0);	  // £1 High Level Switch
...
// club public enemy no.1 initialisation //////////////////////////////////
...
DRIVER_INIT (cpeno1)
{
	...
	Scorpion2_SetSwitchState(5,0,1);	// pay sensor (£1 front)
...
// ROM definition DrWho ///////////////////////////////////////////////////

ROM_START( bfmdrwho )
	ROM_REGION( 0x12000, REGION_CPU1, 0 )	/* 64 + 8k for code */
	ROM_LOAD( "95750288.bin", 0x0000, 0x10000, CRC(fe95b5a5) SHA1(876a812f69903fd99f896b35eeaf132c215b0035))
...
// ROM definition Dutch focus /////////////////////////////////////////////

ROM_START( bfmfocus )
	ROM_REGION( 0x12000, REGION_CPU1, 0 )	/* 64 + 8k for code */
	ROM_LOAD( "focus.bin", 0x0000, 0x10000,  CRC(ddd1a21e) SHA1(cbb467b03642d6de37f6dc204b902f2d7e92230e))	

	ROM_REGION( 0x20000, REGION_SOUND1, 0 ) // samples
	ROM_LOAD( "focsound.bin", 0x00000, 0x20000, CRC(fce86700) SHA1(546680dd85234608c1b7e850bad3165400fd981c))
ROM_END

// ROM definition club public enemy no.1 //////////////////////////////////

ROM_START( cpeno1 )
	ROM_REGION( 0x12000, REGION_CPU1, 0 )	/* 64 + 8k for code */
	ROM_LOAD( "ce1std25p.bin", 0x0000, 0x10000, CRC(2fad9a49) )	

	ROM_REGION( 0x20000, REGION_CPU2, 0 )	
	ROM_LOAD( "cpe1_mtx.bin", 0x0000, 0x10000, CRC(5fd1fd7c) )	

	ROM_REGION( 0x80000, REGION_SOUND1, 0 ) // samples
	ROM_LOAD( "cpe1_snd.bin",	0x00000, 0x80000, CRC(ca8a56bb) )
ROM_END
...
// ROM definition bfmcgslm ////////////////////////////////////////////////

ROM_START( bfmcgslm )
	ROM_REGION( 0x12000, REGION_CPU1, 0 )	/* 64 + 8k for code */
	ROM_LOAD( "95750843.bin", 0x0000, 0x10000,  CRC(e159ddf6) )	

	ROM_REGION( 0x40000, REGION_SOUND1, 0 ) // samples
	ROM_LOAD( "gslamsnd.bin", 0x00000, 0x40000, CRC(9afb8b42) )
ROM_END

// ROM definition luvjub //////////////////////////////////////////////////

ROM_START( luvjub )
	ROM_REGION( 0x12000, REGION_CPU1, 0 )	/* 64 + 8k for code */
	ROM_LOAD( "95750808.bin", 0x0000, 0x10000,  CRC(e6668fc7) )	

	ROM_REGION( 0x20000, REGION_CPU2, 0 )	
	ROM_LOAD( "mtx_ass.bin", 0x0000, 0x10000,   CRC(cfdd7bb2) )	

	ROM_REGION( 0x80000, REGION_SOUND1, 0 ) // samples
	ROM_LOAD( "snd.bin", 0x00000, 0x80000, CRC(19efac32) )
ROM_END

//	   year, name,     parent,	  machine,       input,     init,		monitor, company,    fullname
GAMEL( 1994, bfmdrwho, 0,		  scorpion2,	 drwho,		drwho,		0,		 "BFM",      "Dr.Who The Timelord (UK, Game Card 95-750-288)",GAME_SUPPORTS_SAVE,layout_drwho)
GAME ( 1995, bfmfocus, 0,		  scorpion3,	 scorpion3,	focus,		0,		 "BFM/ELAM", "Focus (Dutch, Game Card 95-750-347)", GAME_NOT_WORKING|GAME_SUPPORTS_SAVE)
GAME ( 1996, bfmcgslm, 0,		  scorpion2,	 bfmcgslm,	bfmcgslm,	0,		 "BFM",		 "Club Grandslam (UK, Game Card 95-750-843)", GAME_NOT_WORKING|GAME_SUPPORTS_SAVE)

GAME ( 1996, luvjub,   0,		  scorpion2_dm01,luvjub,	luvjub,		0,		 "BFM",		 "Luvvly Jubbly (UK Multisite 10/25p, Game Card 95-750-808)", GAME_NOT_WORKING)
GAME ( 1996, cpeno1,   0,		  scorpion2_dm01,cpeno1,    cpeno1,     0,       "BFM",      "Club Public Enemy No.1 (UK, Game Card 95-750-846)", GAME_NOT_WORKING)
