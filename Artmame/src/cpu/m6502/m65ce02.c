...
const char *m65ce02_info(void *context, int regnum)
{
	...
	switch( regnum )
	{
		case CPUINFO_INT_REGISTER+M65CE02_PC: sprintf(buffer, "PC:%04X", r->pc.w.l); break;
		case CPUINFO_INT_REGISTER+M65CE02_S: sprintf(buffer, "S:%04X", r->sp.w.l); break;
		case CPUINFO_INT_REGISTER+M65CE02_P: sprintf(buffer, "P:%02X", r->p); break;
		case CPUINFO_INT_REGISTER+M65CE02_A: sprintf(buffer, "A:%02X", r->a); break;
		case CPUINFO_INT_REGISTER+M65CE02_X: sprintf(buffer, "X:%02X", r->x); break;
		case CPUINFO_INT_REGISTER+M65CE02_Y: sprintf(buffer, "Y:%02X", r->y); break;
		case CPUINFO_INT_REGISTER+M65CE02_Z: sprintf(buffer, "Z:%02X", r->z); break;
		case CPUINFO_INT_REGISTER+M65CE02_B: sprintf(buffer, "B:%02X", r->zp.b.h); break;
		case CPUINFO_INT_REGISTER+M65CE02_EA: sprintf(buffer, "EA:%04X", r->ea.w.l); break;
		case CPUINFO_INT_REGISTER+M65CE02_ZP: sprintf(buffer, "ZP:%04X", r->zp.w.l); break;
		case CPUINFO_INT_INPUT_STATE+M65CE02_NMI_STATE: sprintf(buffer, "NMI:%X", r->nmi_state); break;
		case CPUINFO_INT_INPUT_STATE+M65CE02_IRQ_STATE: sprintf(buffer, "IRQ:%X", r->irq_state); break;
		case CPUINFO_STR_FLAGS:
			sprintf(buffer, "%c%c%c%c%c%c%c%c",
				...
				r->p & 0x01 ? 'C':'.');
			break;
		case CPUINFO_STR_NAME: return "M65CE02";
		case CPUINFO_STR_CORE_FAMILY: return "CBM Semiconductor Group CSG 65CE02";
		case CPUINFO_STR_CORE_VERSION: return "1.0beta";
		case CPUINFO_STR_CORE_CREDITS:
			return "Copyright (c) 1998 Juergen Buchmueller\n"
				"Copyright (c) 2000 Peter Trauner\n"
				"all rights reserved.";
		case CPUINFO_STR_CORE_FILE: return __FILE__;
